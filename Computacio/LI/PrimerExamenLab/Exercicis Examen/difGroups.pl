...
incompatible(S1,S2):- %% returns a pair of students that coincided last year
    ...
    member(S1,T),
    member(S2,T),
    %findall(sst(S1,S2,T),lastYearTeam(T),L),
    %atLeast(1,L),
    true.

%%%%%%  SAT Variables:
satVariable( st(S,T) ):- student(S), team(T). % student S goes to team T
satVariable(sst(S1,S2,T)) :- student(S1), student(S2), S1 < S2, team(T).
...
variables :-
	...
	student(S2),
	S1 <  S2,
	writeClause([-st(S1,T),-st(S2,T),sst(S1,S2,T)]),
	fail.
variables.

assignacio :-
	student(S1),
	findall(st(S1,T),team(T),L),
	exactly(1,L),
	fail.
assignacio.
...
restriccio :-
	team(T),
	findall(sst(S1,S2,T),incompatible(S1,S2),L),
	atMost(1,L),
	fail.
restriccio.
...
displaySol(M) :- nl, incompatible(S1,S2), write(S1), write(" "), write(S2), nl, fail.
