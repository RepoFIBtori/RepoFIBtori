...
%cada habitant té un representant del seu barri
%maxim n representants
%només un representant per cada barri

mateixbarri :- %maxim un representant per barri
	neighbor(N1),
	neighbor(N2),
	N1 < N2,
	neighborNeighborhood(N1,NH),
	neighborNeighborhood(N2,NH),
	writeClause([-chosen(N1),-chosen(N2)]),
	fail.
mateixbarri.
...
representantperhabitant :-
	neighbor(N),
	neighborRepresentedBy(N,L),
	findall(chosen(N1),member(N1,L),K),
	atLeast(1,K),
	fail.
representantperhabitant.
